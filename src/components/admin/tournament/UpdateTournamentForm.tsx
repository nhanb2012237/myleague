...
const UpdateTournamentForm = ({
  ...
  const handleDelete = async () => {
    try {
      setLoading(true);
      const response = await axios.delete('/api/tournament/delete', {
      ...
      if (response.status === 200) {
        ...
        await mutate(
          `/api/tournament/getTournament?tournamentId=${tournamentId}`,
          null,
          false,
        ); // Xóa cache của giải đấu cụ thể
        await mutate('/api/tournaments', undefined, true); // Tải lại danh sách giải đấu
        router.push('/tournament?deleted=true');
      } else {
        throw new Error('Lỗi khi xóa giải đấu');
      }
    } catch (error) {
          ...
          <div className="ml-3 w-full flex-1 md:w-1/2">
            ...
            <div className="flex w-full flex-1 justify-end gap-3">
              ...
              <ConfirmDeletionModal
                ...
                id="Bạn có chắc chắn muốn xóa giải đấu này?"
              />
              <Button
                type="button"
                disabled={uploading}
